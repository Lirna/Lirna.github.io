...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Lirna")[0];
}
...
function showWelcome() {
  ...
  let dist = getDistance(110.31172887, 20.01717009, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("2023-3-25 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Lirna %c 你正在访问Lirnaの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Lirna") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Lirna的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("2023-3-25 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://i.328888.xyz/2023/03/25/iApYFE.png);
    --darkmode-bg:url(https://i.328888.xyz/2023/03/25/iApmSo.png);
